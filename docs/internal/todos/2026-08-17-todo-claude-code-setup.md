...
- Keep the existing `docs/` taxonomy. No migration to a Diátaxis `docs/` tree.
- Guard `dev` + `main` client-side; GitHub enforces nothing on this repo.
- Omit `.mcp.json`.

### Findings that shaped this work

1. **The committed OpenAPI snapshot is 12 paths stale.** `docs/reference/api/lakira-backend-openapi.json` has 18 paths; `lakira-backend/docs/reference/api/lakira-backend-openapi.json` has 30. Missing: `/auth/refresh`, `/auth/verify-email`, `/auth/resend-verification`, `/auth/forgot-password`, `/auth/reset-password`, `/auth/switch-org`, `/organizations/{id}/invites`, `/organizations/{id}/members`, `/invites/accept`, `/memberships/{id}`, `/metric-logs/{metricId}/dummy`, `/admin/_ping`. The frontend type layer has no knowledge of multi-tenancy or the full auth lifecycle, and nothing generates types from the spec.
...
Every rule carries `paths:` frontmatter so it loads only when a matching file is in play, and links to the standards docs under `docs/documentation/` rather than restating them.
...
**Kept:** `sync-api-types` (rewrite of `gen-openapi`), `pre-push` (CI is long and serial), `new-feature` (elaborate module convention, already drifting), `new-route` (**new** — the `@modal` invariant that caused `docs/internal/incidents/fix-metric-modal-routing-20251130.md`).
...
- [x] `docs/internal/todos/2026-08-17-todo-claude-code-setup.md` (this file)
...
- [x] `docs/reference/commands.md` (canonical target)
...
- **Link, never duplicate:** rules link to `docs/documentation/**`; they do not restate it.
...
- `src/components/hoc/withAuth.tsx`, `src/components/providers/HydrateUser.tsx` — **Done 2026-09-22: deleted, not inverted.** `HydrateUser` was referenced nowhere. `withAuth` guarded only `/account`, which `src/proxy.ts` and `(app)/layout.tsx` already gate, and it duplicated the profile fetch `useAuthProfileQuery` makes on that page. Removing an inversion that only ever served dead code is cheaper than inverting it.
- `src/components/layout/Header.tsx`, `src/components/layout/Sidebar.tsx` — app-shell concerns needing auth state; they belong under `src/app/` or need state injected. **Still quarantined** — these two are the whole list now.
