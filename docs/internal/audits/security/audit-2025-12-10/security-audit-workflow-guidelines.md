...
- **Evidence or it didn’t happen**: Store logs, screenshots, tool outputs, and diffs under `docs/internal/audits/security/<cycle>/artifacts/` and reference the artifact path in the audit log.
...
1. **Spin up the audit folder**: `docs/internal/audits/security/audit-2025-12-10/` must contain: plan snapshot, workflow guidelines (this file), threat model, control matrix, audit log, remediation tracker, and `/artifacts`.
