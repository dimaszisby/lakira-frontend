...
- Implementation tasks:
  ...
  - [x] Ensure `withAuth` (or layout-level guard) applies consistently so middleware is not the only protection. _(Handled centrally via `src/app/(app)/layout.tsx`, which redirects unauthenticated requests to `authRoutes.login()`.)_
