...
| --- | --- | --- | --- | --- |
| **OWASP ASVS V1.1 – Secure SDLC** | Governance docs, audit plan, workflow guidelines. | `documents/security/*` | In Progress | Plan + workflow exist, but audit log + threat model only recently seeded. Need recurring cadence + evidence attachments. |
...
| **NIST CSF PR.IP – Protection Processes** | Audit plan & workflow. | `documents/security/security-audit-plan.md`, `security-audit-workflow-guidelines.md` | In Progress | Processes exist; need evidence of execution (logs filled per cycle). |
