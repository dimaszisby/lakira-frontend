...
| Cross-cutting A11y (Integration + E2E) | `documents/tests-plans-and-logs/3-integration-tests/a11y-testing-checklist.md`, `documents/tests-plans-and-logs/4-end-to-end-tests/a11y-e2e-checklist.md`, `documents/documentation/accessibility-guidelines.md` | `Partial` | Medium | Integration-level `jest-axe` assertions are now active on login, metric/log/settings forms, metrics list, dashboard, metric-detail composite, and bottom-navigation baselines; E2E a11y checks are still pending. | Expand `jest-axe` assertions on remaining integration shell/navigation screens (sidebar/layout) and decide E2E a11y gate scope. |
