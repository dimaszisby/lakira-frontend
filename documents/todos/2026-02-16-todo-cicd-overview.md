...
| Cross-cutting A11y (Integration + E2E) | `documents/tests-plans-and-logs/3-integration-tests/a11y-testing-checklist.md`, `documents/tests-plans-and-logs/4-end-to-end-tests/a11y-e2e-checklist.md`, `documents/documentation/accessibility-guidelines.md` | `Partial` | Low-Medium | Initial integration-level `jest-axe` assertion is in place (`LoginForm.int.test.tsx`), but coverage is not broad yet and E2E a11y checks remain planned. | Expand `jest-axe` assertions on key integration screens and decide E2E a11y gate scope. |
