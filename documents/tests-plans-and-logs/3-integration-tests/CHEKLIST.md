...
- [ ] A shared `renderWithProviders` helper exists and is used (React Query, Jotai/store, Theme, Router).
- [ ] MSW is configured for tests:
  - [ ] Global setup/teardown hooks start and stop the MSW server.
  - [ ] Handlers exist for the main API endpoints used by the feature.

<!-- SPECIAL NOTE: Add the actual path(s) to render/test-utils and MSW setup once stabilized. -->
...
- [ ] There is at least **one integration test file** for the feature:
  - e.g. `FeatureName.int.test.tsx` or `page.int.test.tsx`.
...
<!-- SPECIAL NOTE: Decide and document the exact naming pattern (`*.int.test.tsx` or directory-based) in PLAN.md and update examples here. -->
...
- [ ] There is an MSW handler covering the **happy path**.
...
<!-- SPECIAL NOTE: Link to MSW handlers and any custom test QueryClient once their paths are final. -->
...
<!-- SPECIAL NOTE: Document how Next.js routing is mocked in tests (e.g., mock for `next/navigation` or custom hook wrappers). -->
