...
- **Two files are exempt from the layer rule.** `src/components/**` was unmapped in `boundaries/elements` until 2026-08-17, so the `components` boundary never ran and 18 inversions accumulated. The rule is live now, with `Header.tsx` and `Sidebar.tsx` quarantined at the bottom of `eslint.config.mjs` pending a refactor. The list has only ever shrunk: `CategorySelect` and `Visualization` moved into their feature modules on 2026-09-11, and `withAuth.tsx` and `HydrateUser.tsx` were deleted on 2026-09-22. Do not add to that list.
