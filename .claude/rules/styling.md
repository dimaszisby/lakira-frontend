...
- **Button colour tokens fail two WCAG criteria, in both themes** — accepted and recorded in [ADR-0017](../../docs/explanation/decisions/adr-0017-accept-the-button-contrast-deviations.md), which carries the full measured tables.
  - **1.4.3 Contrast (Minimum):** every light-mode variant fails, worst at secondary hover (1.61:1); in dark mode destructive (3.16:1) and two hover states fail. `--button-destructive-fg` is never overridden for dark, which is why it is not a light-mode-only problem.
  - **1.4.11 Non-text Contrast:** every button boundary fails against every light-mode surface (1.64–3.00:1). Most acute on secondary, whose border is its only affordance.
  - Repointing to the next ramp step does **not** fix it — the 700 steps reach only 3.12:1 and 4.39:1 under white text. Changing them is a brand decision, not a refactor. `jest-axe` cannot catch this: colour contrast is not evaluated in jsdom.
