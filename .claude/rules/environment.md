...
Server-only vars (no prefix) are readable in route handlers, `src/proxy.ts` (the edge gate), server components, and `next.config.ts`. That is where anything sensitive belongs.

## The matrix

| Var | Public | Used by |
|---|---|---|
| `API_URL` | server | `api/proxy/[...path]`, `api/auth/{logout,revive,session}` — the backend base URL |
...
- `src/app/api/auth/login/route.ts` was deleted on 2026-09-12. It had been unreachable since it was written — login goes through `/api/proxy/auth/login` — so it was a second, divergent login implementation. Its refresh-cookie capture moved into the proxy, which is where login actually runs. (It was also the route that once built `undefined/auth/login`.)
