...
export const invalidateMetricLists = async (qc: QueryClient) => {
  // Offset-based
  await qc.invalidateQueries({ queryKey: metricsKeys.lists(), exact: false });
  // Cursor-based (pages + infinite share the same root)
  await qc.invalidateQueries({ queryKey: metricsKeys.cursor.root(), exact: false });
};

export const invalidateMetricDetail = async (qc: QueryClient, metricId: string) => {
...
export const getMetricDetailVM = (
  ...
  includes: IncludeKey[] = ["category", "settings"],
  logsLimit?: number,
) => qc.getQueryData<MetricDetailCompositeVM>(detailKey(metricId, includes, logsLimit));
...
export const setMetricDetailVM = (
  ...
  includes: IncludeKey[] = ["category", "settings"],
  logsLimit?: number,
) => qc.setQueryData<MetricDetailCompositeVM>(detailKey(metricId, includes, logsLimit), next);
